...
def return_number_of_variables():
    ...
    st.markdown("""
    Most often, you do not need 2 million datapoints from hundreds of variables to achieve your goals.
    ...
    Therefore, please select the number of variables in your dataset:
    """)
    option = st.selectbox(
    ...
    if option == '<10':
        ...
        If not, then that means that you have to check with your data supplier why some specific variables are not aparant in your dataset.
        """)
        
    if option == '10-30':
        st.warning('You are in the safe zone, but keep in mind that the number of variables can be reduced further')
        st.markdown("""
        If your dataset contains 10-30 variables, there is a good chance that you have just enough variables to perform an analysis, but not too many which will make it hard to determine which variables to use.
        Therefore, we advice to reduce your dataset to less variables if your dataset is closer to 30 than 10 variables. 
        After all, less variables is better (if they are meaningfull and unique).
        """)
    if option == '31+':
        ...
        st.markdown('''Most likely, quite a lot of your variables are set variables, which means they only monitor the given settings of the machine.
    ...
    If this occurs, we advice to use another tool in our Di-Plast project, called the [Sensoring tool](https://share.streamlit.io/skz-digi/diplastsensorselection/updated/main.py).
    ''')
